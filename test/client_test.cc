...
/* TODO(oschaaf): Fix this in the python integration tests.
TEST_F(ClientTest, CipherSuiteSelection) {
  ...
  Main program_128(Nighthawk::Client::TestUtility::createOptionsImpl(
      R"CFG(foo --cluster-config
{tls_context:{common_tls_context:{tls_params:{cipher_suites:["-ALL:ECDHE-RSA-AES128-SHA"]}}}} --rps
1 --concurrency 1 --output-format yaml --duration 1 https://www.microsoft.com/ )CFG"));
  EXPECT_TRUE(program_128.run());

  Main program_256(Nighthawk::Client::TestUtility::createOptionsImpl(
      R"CFG(foo --cluster-config
{tls_context:{common_tls_context:{tls_params:{cipher_suites:["-ALL:ECDHE-RSA-AES256-GCM-SHA384"]}}}}
--rps 1 --concurrency 1 --output-format yaml --duration 1 https://www.microsoft.com/ )CFG"));
